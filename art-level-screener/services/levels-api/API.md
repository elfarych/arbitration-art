...
| `tolerancePct` | number ≥ 0 | — | Погрешность зоны касания напрямую в % от цены (±`tolerancePct`). При `>0` **переопределяет** `natrMultiplier`; `0`/нет — расчёт от NATR |
...
`minVolume`, `natrMultiplier` / `tolerancePct`, `minGap` влияют на расчёт; остальные параметры —
post-фильтры/сортировка/пагинация поверх результата. `tolerancePct` задаёт **единую** ширину
зоны для всех монет независимо от их NATR (для экрана из разных монет это нельзя выразить через
один `natrMultiplier`), поэтому процентный режим — отдельный серверный параметр.
...
| `tolerancePct` | number ≥ 0 | — | Погрешность зоны касания/пробоя напрямую в % от цены (±`tolerancePct`). При `>0` **переопределяет** `natrMultiplier`; `0`/нет — расчёт от NATR. В ответе `params.natrMultiplier` тогда несёт эффективный множитель (`tolerancePct/natr` для этой монеты) |
