...
первый пробой — **первую свечу, закрывшуюся за уровнем** (по закрытию, не по
фитилю; тычок-фейкаут не считается), проход слева направо → по трейдам найти **момент пересечения
...
| `candles` | integer ≥ 61 | env `ANALYSIS_DEFAULT_CANDLES` (`1000`) | Сколько свечей грузить; сверху ограничено `ANALYSIS_MAX_CANDLES` (`3000`). Значения >1500 грузятся из Binance пагинацией |
