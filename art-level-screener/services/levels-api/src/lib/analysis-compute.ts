...
Level detection reuses `../levels` (findExtrema + countTouches + NATR), so the
levels found here cannot diverge from `computeScreener`. The breakout scan and
the trade-based match are analysis-specific. Trades give sub-candle timing that
1m+ candles cannot: from the moment a trade crosses the level zone to the moment
price has moved `minMovePct` beyond it, at most `maxBreakoutSeconds` may pass. */

import type { AnalysisBreakoutType, AnalysisResponseType } from '../schemas/level';
 ...
 *
 * Returns `null` when there are not enough candles to detect any level (the
 * route maps this to 404). Binance HTTP errors propagate to the caller.
 */
export async function computeAnalysis(
  ...
  const breakouts: AnalysisBreakoutType[] = [];
...
function isEvaluated(breakout: AnalysisBreakoutType): boolean {
  return breakout.reason !== 'no_trades';
}

function buildSummary(breakouts: readonly AnalysisBreakoutType[]): AnalysisResponseType['summary'] {
