...
| GET | `/analysis/:tf/:symbol` | Анализ пробоев уровней по монете: свечи+трейды **напрямую с Binance REST**, уровни тем же кодом, проверка пробоев по трейдам |
...
### Анализ пробоев `/analysis/:tf/:symbol`
...
- **Страница монеты + анализ**: `/analysis/:tf/:symbol` вызывает **Django**
  (`apps.levels`), которое проксирует расчёт и сохраняет результат в БД под
  пользователем (история анализов). Фронт (`/levels/:symbol`) ходит в Django
  (`/api/levels/analyses/`), а не сюда напрямую. См. `arbitration-art-django/DOCS.md` §10A.
