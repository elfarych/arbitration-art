...
- **Ценовые уведомления** — Django `apps.levels` `PriceNotification` (**коллекция** на пользователя, адресуется по id; one-shot). Три стороны: (1) user CRUD `/api/levels/price-notifications/` (`IsAuthenticated`) ↔ фронт `priceNotificationsApi` + `usePriceNotificationsStore` + вкладка «По цене» в `LevelsNotificationsDialog` + правый клик на графике (`LevelChartCard` + `useChartRightClick` + `PriceNotificationsOverlay`); (2) сервисный фид `GET /api/levels/service/price-notifications/` (`ServiceTokenOnly`, отдаёт активные + `chatId` владельца из его `LevelNotificationConfig`) ↔ сервис `level-notifier` (`getActivePriceNotifications`); (3) one-shot отключение `POST /api/levels/service/price-notifications/<id>/trigger/` ↔ `level-notifier` (`markPriceNotificationTriggered`). Меняешь форму — синхронизируй Django-сериализаторы (`PriceNotificationSerializer`/`ServicePriceNotificationSerializer`), фронтовый `PriceNotification`/`PriceNotificationInput` и `level-notifier/src/services/django-client.ts`, плюс `DOCS.md` всех трёх. `direction` (`above`/`below`) вычисляется фронтом по позиции клика; ценовому проходу расчёт уровней не нужен — он читает `binance-futures:price:<SYM>` коннектора (universe-gap: монеты вне вселенной скринера алерт не сработают).
