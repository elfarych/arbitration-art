...
| `ORDERBOOK_MAX_SKEW_MS` | `2000` | Максимальная разница между `localTimestamp` primary и secondary snapshot-ов в одном решении. Бортит spread-сигналы, когда одна нога отстаёт от другой. `0` — выключить только этот чек, оставив max-age. |
...
- Последовательно вызывает `setIsolatedMargin`, `setLeverage`, `fetchTicker`, считает количество как `(margin * leverage) / lastPrice` и округляет через `getMarketInfo().stepSize`.
...
6. Если `trade_mode === 'real'`: на каждой ноге параллельно через `Promise.allSettled([setIsolatedMargin, setLeverage])`; обе ноги параллельно через `Promise.all`; rejected задачи логируются как WARN.
...
- `createMarketOrder` — `POST /fapi/v1/order` с `newOrderRespType=RESULT`; быстрый retry `/fapi/v1/order` через 100ms при `avgPrice=0` / non-terminal status.
...
- `createMarketOrder` — `POST /v5/order/create` (category=linear, positionIdx=0); fast retry 150ms на `fetchOrderRaw` для получения `avgPrice`/`cumExecQty`.
...
- `createMarketOrder` — `POST /futures/usdt/orders` с `tif=ioc` и `price="0"`. Size в контрактах через `quanto_multiplier`. Positive=long, negative=short. Fast retry 150ms на `GET /futures/usdt/orders/{id}` для `fill_price`.
...
Recovery открытых сделок (`Engine.startBot` → `api.getOpenTrades(botId)` → `BotTrader.restoreOpenTrades`) фильтрует по `bot_id` на стороне Django и затем по `coin` в памяти. Остаются два edge-case:

- Дубли open-записей на одном `(bot_id, coin)`: `restoreOpenTrades` берёт первый matching `Trade`.
