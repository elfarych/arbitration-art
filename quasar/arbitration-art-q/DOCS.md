...
- `src/components/levels/PriceNotificationsPanel.vue` — вкладка «По цене»: список ценовых алертов (символ, стрелка ↑/↓ + целевая цена, статус «при росте/падении до цены» / «сработал {время}»), **удаление с подтверждением** (`$q.dialog` confirm), тумблера вкл/выкл нет. Внизу — общий Telegram-блок (`TelegramSettings`, `v-model:chat-id` от диалога) + кнопка «Сохранить» (`notificationsStore.save()` сохраняет `chatId`, сохраняя поля уровней). Создание алертов — только правым кликом по графику (направление выводится из позиции, см. §25B.1). См. §25B.9.
...
- **Вкладка «По цене»** (`PriceNotificationsPanel.vue`, **первая вкладка**): список алертов; удаление каждого — **с подтверждением** (`$q.dialog`), тумблера вкл/выкл нет. Создание — только через правый клик по графику (направление выводится из позиции относительно текущей цены, поэтому условие срабатывания ложно при создании; ручного ввода направления нет). Статус «сработал {время}» по `triggeredAt`. Внизу — общий Telegram-блок + кнопка «Сохранить», которая сохраняет `chatId` (поля уровней при этом не меняются — берутся из текущего конфига).
