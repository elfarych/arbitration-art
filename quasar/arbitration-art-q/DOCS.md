...
Options Store `levels`. State: `timeframes`, `timeframe`, `rawItems` (свечи последнего запроса), `serverTotal`, `page` (1-based), `pageSize` (= `columns × rows`, дефолт `LEVELS_DEFAULT_PAGE_SIZE = 6`), `pinFavorites` (закрепить избранные сверху), параметры расчёта `minVolume` / `natrMultiplier` / `minGap` (дефолты `LEVELS_DEFAULT_MIN_VOLUME = LEVELS_MIN_VOLUME = 50_000_000` — пол 50M, ниже задать нельзя / `LEVELS_DEFAULT_NATR_MULTIPLIER = 0.3` / `LEVELS_DEFAULT_MIN_GAP = 12`; NATR/gap совпадают с env-дефолтами сервиса, объём флорится клиентом), `loading`, `error`, `fetchedAt`. Getters: `total` (`pinFavorites` → `rawItems.length`, иначе `serverTotal`), `pageCount = ceil(total / pageSize)`, **`entries`** (что рендерить на текущей странице). Actions: `fetchTimeframes` (выбирает первый ТФ, если текущий невалиден), `fetchScreener`, `setTimeframe` (сброс на стр. 1), `setPage`, `setPageSize` (смена сетки → сброс на стр. 1 + перезапрос), `setPinFavorites(pin)` (сброс на стр. 1 + перезапрос), `setParams` (применяет объём/погрешность/minGap, сброс на стр. 1 + перезапрос; no-op, если ничего не изменилось). Ошибки — через общий `extractApiErrorMessage` (§утиль).
