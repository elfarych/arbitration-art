...
- `src/components/levels/LevelsNotificationsDialog.vue` — диалог уведомлений (`v-model` показа), **две вкладки** (`q-tabs` + `q-tab-panels`): «По уровням» (`LevelNotificationsForm`) и «По цене» (`PriceNotificationsPanel`). При открытии грузит оба стора (`useNotificationsStore().load()`, `usePriceNotificationsStore().load()`). См. §25B.8, §25B.9.
- `src/components/levels/LevelNotificationsForm.vue` — форма конфига уведомлений по уровням (вкладка «По уровням», вынесена из диалога). Поля: включить/только избранные (`q-toggle`), таймфрейм (`q-select` из `levelsStore.timeframes`), объём в M$, погрешность (NATR), свечей между касаниями, режим дистанции `% / NATR` (`q-btn-toggle`) + значение, `chatId`. Локальная копия формы ресинкается из стора на открытие диалога (проп `open`); сохранение → `notificationsStore.save()` + `@saved` (диалог закрывается). Объём редактируется в млн (×1e6), как в `LevelsFilters`. См. §25B.8.
- `src/components/levels/PriceNotificationsPanel.vue` — вкладка «По цене»: список ценовых алертов (символ, стрелка ↑/↓ + целевая цена, статус «при росте/падении до цены» / «сработал {время}»). Единственное действие — **удаление с подтверждением** (`$q.dialog` confirm); тумблера вкл/выкл нет. Создание — только правым кликом по графику (направление выводится из позиции, см. §25B.1). Подсказка настроить Telegram `chatId` во вкладке «По уровням», если он пуст. См. §25B.9.
...
- **Форма** (`LevelNotificationsForm.vue`, вкладка «По уровням»): локальная копия формы, ресинк из стора на каждое открытие диалога (проп `open`; правки отбрасываются при «Отмена»). Поля 1:1 с моделью — включить, только избранные, таймфрейм (из `levelsStore.timeframes`), объём (M$ → raw USDT ×1e6), погрешность (NATR), свечей между касаниями, режим дистанции `% / NATR` + значение, `chatId`. В секции Telegram — инструкция и кнопка-ссылка на бота; адрес **захардкожен** в `LevelNotificationsForm.vue` (`https://t.me/Brakeoutautobot`, `@username` для подписи парсится из URL). Должен совпадать с ботом, чьим токеном шлёт `level-notifier` (`TELEGRAM_BOT_TOKEN`). Валидация: `natrMultiplier>0`, `minGap>=1`, `minVolume>=50M` (пол `LEVELS_MIN_VOLUME`; дефолт нового конфига тоже 50M), `distanceValue>0`, непустой ТФ. Сохранение → `notificationsStore.save()` (PUT) → `@saved` → закрытие; ошибка показывается баннером, диалог не закрывается.
...
- **Отрисовка на графике.** Активные (`enabled`) алерты монеты рисуются **жёлтой пунктирной линией** (`LineStyle.Dashed`, `applyPriceNotifications`, серии в `Map` по id) от левого края до правого, рядом с лучами уровней. Поверх — HTML-бейдж (`PriceNotificationsOverlay`): **колокольчик + цена + крестик**, позиционируется по `priceToCoordinate`; пересчёт позиции на pan/zoom (`subscribeVisibleLogicalRangeChange`), наведении (`subscribeCrosshairMove`), live-свече и ресайзе (`ResizeObserver`). **Крестик** — мгновенное удаление (без подтверждения; confirm — только во вкладке). **Перетаскивание** бейджа по вертикали меняет целевую цену: линия следует за курсором live (`coordinateToPrice` + `setData` конкретной серии), на отпускании — `store.update(id, {targetPrice, direction})` (направление переинферится по новой цене). Сработавшие (`enabled=false`) не рисуются. Очистка серий/листенеров — при размонтировании.
- **Вкладка «По цене»** (`PriceNotificationsPanel.vue`): список алертов; единственное действие — **удаление с подтверждением** (`$q.dialog`), тумблера вкл/выкл нет. Создание — только через правый клик по графику (направление выводится из позиции относительно текущей цены, поэтому условие срабатывания ложно при создании; ручного ввода направления нет). Статус «сработал {время}» по `triggeredAt`. `chatId` берётся из конфига §25B.8 (подсказка настроить, если пуст).
