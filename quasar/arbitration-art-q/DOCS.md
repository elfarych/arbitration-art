...
- `src/components/levels/LevelsAnalysisDialog.vue` — диалог настроек анализа (7 параметров), `v-model` показа + `@run` с настройками. У поля «Свечей между касаниями» — подсказка (`hint`) с эквивалентным временем (`gapHint` = `≈ <время>`, считается от выбранного в диалоге ТФ через общий хелпер `timeframe.ts`).
...
- `src/components/levels/MiniBreakoutChart.vue` — переиспользуемый мини-график свечей (свечи + price-line уровня + маркеры) для диалога пробоя. Линейка по Shift — через `useChartRuler` (см. §25B.5); секунд-на-бар выводится из шага свечей (нет единого ТФ-пропа).
- `src/components/levels/MiniTickChart.vue` — мини-график линии тиков (line-series по трейдам + price-line уровня + маркеры пробоя/цели) для диалога пробоя. Линейка по Shift — через `useChartRuler`; секунд-на-бар = медиана шага тиков.
...
**Навигация.** Клик по шапке карточки (`LevelChartCard` эмитит `@open` с символом) ведёт на `/levels/:symbol`; страница скринера в обработчике (`onOpenCoin`) делает `router.push({ path: '/levels/<symbol>', query: { tf, natrMultiplier, minGap } })` — переносит текущий ТФ **и параметры детекции из фильтров** (`store.natrMultiplier`/`store.minGap`), чтобы график на детальной странице показал те же уровни. `minVolume` не передаётся — это фильтр вселенной, на уровни одной монеты не влияет. `LevelChartCard` навигацию не выполняет сам (остаётся переиспользуемым; на детальной странице `@open` не слушается).
...
**Верхний график (`loadChart`).** Параметры берутся из query (`tf`, `natrMultiplier`, `minGap`), с фолбэком на `store.*`/`LEVELS_DEFAULT_*`. Источник: ссылка из Telegram-уведомления (`level-notifier` кладёт параметры конфига в URL) или переход со скринера (параметры из фильтров, см. «Навигация»). Запрос — `levelsApi.screener(tf, { search: symbol, natrMultiplier, minGap, minVolume: 0, limit: 20 })`, из ответа берётся `items.find(symbol === …)`. **`minVolume: 0` обязателен** — иначе объёмный пре-фильтр мог бы исключить саму просматриваемую монету. Если монета не найдена/404 — показывается заглушка «Нет данных по уровням». Готовый `ScreenerEntry` отдаётся в переиспользуемый `LevelChartCard` (свечи + лучи уровней + live-WS, как на скринере).
...
1. **На ТФ анализа** (сверху) — `MiniBreakoutChart`, свечи вокруг пробоя через `fetchKlines(symbol, timeframe, …, endTime=breakoutCandleTime + N·ТФ)`; price-line на цене уровня + маркер-стрелка на свече пробоя.
2. **Посекундный** (снизу слева) — `MiniBreakoutChart`, 1s-OHLC из `aggTrades`; price-line уровня + маркеры пересечения (`crossTime`) и достижения цели (`reachTime`). Маркеры ставятся в поле **со стороны, противоположной движению** (чтобы не перекрывать цену), мелкие; при `elapsed=0` (cross и reach в одной секунде) — один объединённый маркер «пробой → цель».
3. **Тиковый** (снизу справа) — `MiniTickChart`, **линия по трейдам** (line-series, один пойнт = один трейд) из тех же `aggTrades`; price-line уровня + те же маркеры пробоя/цели. Время тика = `T/1000` (секунды с миллисекундной дробью), строго возрастающее (дубликаты сдвигаются на 1мс) — lightweight-charts принимает дробные `UTCTimestamp`, поэтому всплески трейдов видны с реальным распределением во времени. Маркеры line-серии должны стоять на существующем пойнте, а `crossTime`/`reachTime` (целые мс) с тиками не совпадают — поэтому диалог считает `tickMarkers`, привязывая cross/reach к **ближайшему тику** (`nearestTickTime`).
