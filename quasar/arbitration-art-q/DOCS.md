...
- `src/components/levels/LevelsNotificationsDialog.vue` — диалог настроек Telegram-уведомлений по уровням (`v-model` показа). Поля: включить/только избранные (`q-toggle`), таймфрейм (`q-select` из `levelsStore.timeframes`), объём в M$, погрешность (NATR), свечей между касаниями, режим дистанции `% / NATR` (`q-btn-toggle`) + значение, `chatId`. Локальная копия формы ресинкается из стора на открытие; сохранение → `notificationsStore.save()`. Объём редактируется в млн (×1e6), как в `LevelsFilters`. См. §25B.8.
