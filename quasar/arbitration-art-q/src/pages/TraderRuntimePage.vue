  ...
  <q-page padding>
    <div class="row items-center justify-between q-mb-lg">
      ...
      <div class="row items-center q-gutter-sm">
        ...
        <q-btn v-if="!runtimeConfig" color="primary" text-color="white" no-caps icon="add" label="Создать" @click="openCreateDialog" />
      </div>
    </div>
          ...
          <q-card-section class="row items-start justify-between q-col-gutter-md">
            ...
            <div class="col-12 col-md-auto row q-gutter-sm">
              ...
              <q-btn
                ...
                @click="stopSelected"
              />
              <q-btn no-caps outline color="primary" icon="sync" label="Sync" :disable="!runtimeConfig.is_active" :loading="saving" @click="syncSelected" />
          ...
          <q-card-section>
            <div class="row q-col-gutter-md">
              ...
              <div class="col-6 col-md-3">
                ...
                <div class="text-weight-bold" :class="runtimeConfig.sync_status === 'failed' ? 'text-negative' : ''">{{ runtimeConfig.sync_status }}</div>
              </div>
              <div class="col-6 col-md-3">
              ...
              <div class="col-6 col-md-3">
                <div class="text-caption text-grey-5">Top pairs</div>
                ...
                <q-card dark flat bordered class="bg-info full-height">
                  <q-card-section>
                    ...
                    <div v-if="exchangeHealth">
                      <div v-for="item in exchangeHealth.exchanges" :key="item.exchange" class="row items-start justify-between q-mb-sm">
                        <div>
                        ...
                        <q-badge :color="item.available ? 'positive' : 'negative'" :label="item.available ? 'OK' : 'FAIL'" />
                      </div>
                    </div>
                  ...
                  <q-card-section>
                    ...
                    <div v-if="systemLoad">
                      <div class="row justify-between q-mb-xs">
                        ...
                        <span :class="systemLoad.risk_locked ? 'text-negative' : 'text-positive'">{{ systemLoad.runtime_state }}</span>
                      </div>
                      <div class="row justify-between q-mb-xs">
                        <span class="text-grey-5">CPU</span>
                        <span>{{ systemLoad.cpu_percent }}%</span>
                      </div>
                      <q-linear-progress :value="systemLoad.cpu_percent / 100" color="primary" track-color="grey-9" class="q-mb-sm" />
                      <div class="row justify-between q-mb-xs">
                        <span class="text-grey-5">Memory</span>
                        <span>{{ systemLoad.memory_used_percent }}%</span>
                      </div>
                      <q-linear-progress :value="systemLoad.memory_used_percent / 100" color="warning" track-color="grey-9" />
                    </div>
                    <div v-else class="text-grey-5">Нет данных</div>
                  </q-card-section>
                </q-card>
                ...
                <q-card dark flat bordered class="bg-info">
                  <q-card-section>
                    ...
                    <div v-if="activeCoins?.active_coins.length" class="row q-gutter-xs">
                      <q-chip v-for="coin in activeCoins.active_coins" :key="coin" dense color="primary" text-color="white">{{ coin }}</q-chip>
                    </div>
...
const pnlColumns: QTableColumn[] = [
  ...
  { name: 'pnl', label: 'PnL %', field: 'estimated_pnl_percentage', align: 'right', format: (value: number | null) => formatNullablePercent(value) },
  { name: 'pnl_usdt', label: 'PnL USDT', field: 'estimated_pnl_usdt', align: 'right', format: (value: number | null) => formatNullableNumber(value) },
...
const tradeColumns: QTableColumn[] = [
  ...
  { name: 'open_spread', label: 'Open %', field: 'open_spread', align: 'right', format: (value: string) => `${formatNumber(value)}%` },
  { name: 'profit_usdt', label: 'Profit USDT', field: 'profit_usdt', align: 'right', format: (value: string | null) => formatNullableNumber(value) },
  { name: 'profit_percentage', label: 'Profit %', field: 'profit_percentage', align: 'right', format: (value: string | null) => formatNullablePercent(value) },
